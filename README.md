```markdown
# 🍎 Classificador de Frutas com Machine Learning

Este projeto utiliza um modelo de **Árvore de Decisão** para identificar tipos de frutas com base em suas características físicas.

## 📊 O Modelo
O modelo foi treinado utilizando a biblioteca `scikit-learn`. Abaixo está a representação visual de como a árvore toma as decisões:

## 🛠️ Tecnologias Utilizadas
* **Python 3.13**
* **Pandas:** Manipulação de dados.
* **Scikit-Learn:** Inteligência Artificial.
* **Matplotlib:** Visualização gráfica.
* **Openpyxl:** Leitura de arquivos Excel.

## 📁 Estrutura do Projeto
* `dados/`: Base de dados (Ignorada no Git).
* `frutas.py`: Script principal.
* `.gitignore`: Configuração de arquivos ignorados.
* `README.md`: Documentação.

## 🚀 Como executar
1. Instale as dependências:
   ```bash
   pip install pandas scikit-learn matplotlib openpyxl
